...
const History: React.FC = () => {
  ...
  const getStatusColor = (status: Alert["status"]) => {
    switch (status) {
      ...
      case "false-alarm": return "bg-gray-500";
    }
  };
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mb-4">
        <p className="text-gray-600 dark:text-gray-400">
          Showing {filteredAlerts.length} {filteredAlerts.length === 1 ? 'event' : 'events'}
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
        {filteredAlerts.length > 0 ? (
            ...
            {filteredAlerts.map((alert) => (
              <div key={alert.id} className="flex items-start gap-4">
                ...
                <div className="flex flex-col items-center">
                  <div className={`w-3 h-3 rounded-full ${getStatusColor(alert.status)} mt-1.5`}></div>
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap justify-between gap-2 mb-2">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs flex items-center gap-1 ${
                        ...
                        <span className={`inline-block w-2 h-2 rounded-full ${getStatusColor(alert.status)}`}></span>
                    ...
                    <p className="text-sm mb-2">
                      <span className={`font-medium ${
                        alert.confidence > 0.8 ? "text-red-500" : 
                        alert.confidence > 0.7 ? "text-yellow-500" : 
                        "text-gray-500"
                      }`}>
          ...
          <div className="py-12 text-center">
            <p className="text-gray-500">No detection events match your filters</p>
            <button 
              onClick={() => {
                setDateFilter("all");
